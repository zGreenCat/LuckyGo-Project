...
<form id="regForm" class="max-w-sm mx-auto bg-gray-100  p-9 rounded-lg shadow-lg" method="POST" action="{{route('sorter.data')}}"  novalidate>
    <h1 class="text-center mb-8" style="font-size: 18px; font-weight: bold;">Cambio de datos personales</h1>
    <p class="mb-3 font-medium">Datos actuales: </p>
    <label class="block text-gray-900 dark:text-white">Nombre: {{auth()->user()->name}}</label>
    <label class="block mb-5 text-gray-900 dark:text-white">Edad: {{auth()->user()->age}}</label>
  @csrf
    <div class="mb-2">
    ...
    <div class="mb-5">
      <label for="age" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nueva edad</label>
